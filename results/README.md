...
crossed, and the cheapest thing it can do is cross one early and pay for
the shot anyway.
With only two actions the boundary rule was merely suboptimal; with three it
is *inadmissible* at small w, beaten by doing nothing. This is the one place
in the whole comparison where the learned policy is not merely competitive
with the exact boundary but categorically better, and it is structural
rather than statistical — no amount of tuning (L, offset, deadline) fixes it.
